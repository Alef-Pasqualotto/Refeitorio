...
switch ($pesquisa) {
    ...
    case 1: //selecionar sem data específica
        $query = $conn->query('SELECT cardapio.cardapio_id, item.descricao as nome_do_prato, GROUP_CONCAT(ingrediente.nome SEPARATOR ", ") 
            ...
            SUM(CASE WHEN ingrediente_item.item_id = ingrediente_item.item_id THEN calorias ELSE 0 END) as soma_das_calorias, item.item_id
            ...
            WHERE cardapio.dt_exclusao = 0000-00-00 OR cardapio.dt_exclusao > CURRENT_TIMESTAMP() AND item.dt_exclusao IS NULL
    ...
    case 2: //selecionar por data ex:0000-00-00
        $query = $conn->prepare('SELECT cardapio.cardapio_id, item.descricao as nome_do_prato, GROUP_CONCAT(ingrediente.nome SEPARATOR ", ") 
            as ingredientes, DATE_FORMAT(cardapio.dt,"%d/%m/%Y") as data_formatada, (CASE WHEN cardapio.tipo = 1 THEN "Café da Manhã" WHEN cardapio.tipo = 2 THEN "Almoço" WHEN cardapio.tipo = 3 THEN "Janta" END) as tipo_formatado, tipo, usuario_id, usuario.nome, usuario.crn,
